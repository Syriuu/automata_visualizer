...
const TRANSITION_COLOR = 'rgba(255,255,255,1)'
...
export default class VisualFSA extends EventHandler {
    ...
    /**
     * Rename a state in the VisualFSA and update all transitions
     * @param {String} oldLabel The current state label
     * @param {String} newLabel The new state label
     */
    renameNode (oldLabel, newLabel) {
        if (!this.fsa.states.includes(oldLabel)) { throw new UnknownStateError(oldLabel) }
        
        // Let the FSA rename the state and its transitions
        this.fsa.renameState(oldLabel, newLabel)

        // Update the visual node
        const node = this.getNode(oldLabel)
        node.label = newLabel
        
        // Update transitionText keys for all nodes
        for (const n of this.nodes) {
            if (n.transitionText[oldLabel]) {
                n.transitionText[newLabel] = n.transitionText[oldLabel]
                delete n.transitionText[oldLabel]
            }
        }
        ...
        for (const node of this.nodes) {
            ...
            if (!this.isDFA) {
                circle.addEventListener('edit', e => {
                    ...
                    editMenu.addEventListener('renamestate', () => {
                        const newName = prompt('Enter a new name for the state:', node.label)
                        if (newName && newName.trim().length > 0 && newName.trim() !== node.label) {
                            try {
                                this.renameNode(node.label, newName.trim())
                                this.render()
                            } catch (err) {
                                alert(err.message)
                            }
